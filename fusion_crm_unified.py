...
class FusionCRMUnified:
    ...
    def show_unified_dashboard(self):
        ...
        if st.session_state.current_view == 'crm':
            self.show_crm_page()  # render_crm_page → show_crm_page
            return
        elif st.session_state.current_view == 'email':
            self.show_email_page()  # render_email_page → show_email_page
            return
        ...
        **現在の推奨ワークフロー:**
        ...
        1. **CRMで企業管理** → `streamlit run fusion_crm_main.py`
           - 企業データの検索・編集
           - ステータス管理
           - Google Sheets同期
        
        2. **メールで配信実行** → `streamlit run email_webapp.py`
           - AI生成メール作成
           - 一括配信
           - 送信結果追跡
        ...
        with col1:
            st.success("✅ CRMシステム: 利用可能")
            st.caption("fusion_crm_main.py")
        
        with col2:
            st.success("✅ メールシステム: 利用可能")
            st.caption("email_webapp.py")
